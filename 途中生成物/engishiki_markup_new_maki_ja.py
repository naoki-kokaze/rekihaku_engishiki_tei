...
# 巻11.csv
output_filename = f'engishiki_v{file_volume}_ja.xml'
# engishiki_v11.xml
...
for content in contents_list2:
    ...
    result.write(f'<div ana="{shiki_name}" n="{jou_no}" subtype="項" type="条"><head ana="{jou_name}"/><note type="summary">条文概要</note>\n')
    
    kous = int(content[-1])
    for kou in range(kous):
        item_id = f'item{shiki_no.zfill(2)}{content[2]}{jou_no_raw.zfill(3)}{str(kou+1).zfill(2)}'
        result.write(f'<p ana="項" corresp="engishiki.xml#{item_id} engishiki_en.xml#{item_id}" xml:id="{item_id}">本文</p>')
    result.write(f'</div>')
