...
def update_p_tag_text(element, replacements):
    ...
    for p in element.xpath('.//tei:p', namespaces=namespaces):  # <p>タグを対象に
        for child in p.iter():  # <p>タグ内のテキストを対象に
            ...
            if child.tag != '{http://www.tei-c.org/ns/1.0}placeName' and child.text:
                # 置換処理
                child.text = unescape(replace_text(child.text, replacements))
            if child.tag != '{http://www.tei-c.org/ns/1.0}placeName' and child.tail:  # tailも同様に処理
                # 置換処理
...
def main():
    ...
    output_path = xml_path.replace('.xml', '_place.xml')
